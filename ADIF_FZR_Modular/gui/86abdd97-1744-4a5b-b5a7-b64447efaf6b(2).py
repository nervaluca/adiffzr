...
class ADIFtoPDFApp(ctk.CTk):
    ...
    def apri_aggiungi_qso(self):
        ...
        dlg.geometry("1180x820")
        dlg.resizable(True, True)
        dlg.minsize(1050, 700)
        ...
        qb_side = ctk.CTkFrame(body, fg_color=("#EEF3F8", "#141C25"), corner_radius=10, width=330, border_width=1, border_color=("#D5DEE8", "#26384A"))
        form = ctk.CTkScrollableFrame(body, fg_color=("#F8FAFC", "#101820"), corner_radius=10, border_width=1, border_color=("#D9E2EC", "#26384A"))
        form.grid_columnconfigure(0, minsize=175)
        form.grid_columnconfigure(1, minsize=175)
        ...
        e_call = ctk.CTkEntry(form, width=165, height=32, placeholder_text=T("aq_ph_call"), font=ctk.CTkFont(size=12, weight="bold"))
        e_call.grid(row=4, column=0, sticky="w", pady=(0, 3))
        e_country = ctk.CTkEntry(form, width=165, height=32, placeholder_text=T("aq_ph_country"))
        e_country.grid(row=4, column=1, sticky="w", padx=(8, 0), pady=(0, 3))
        ...
        qb_slot = ctk.CTkLabel(qb_side, text="", anchor="w", justify="left", wraplength=284,
                               font=ctk.CTkFont(size=12, weight="bold"))
        qb_slot.pack(fill="x", padx=10, pady=(0, 4))
        qb_summary = ctk.CTkLabel(qb_side, text="", anchor="w", justify="left",
                                  wraplength=284, font=ctk.CTkFont(size=11))
        ...
        cb_banda = ctk.CTkOptionMenu(form, values=BANDE, variable=var_banda, width=165, height=32,
                                      command=_su_cambio_banda)
        cb_banda.grid(row=7, column=0, sticky="w", pady=(0, 3))
        e_freq = ctk.CTkEntry(form, width=165, height=32)
        e_freq.grid(row=7, column=1, sticky="w", padx=(8, 0), pady=(0, 3))
        ...
        cb_modo.grid(row=9, column=0, sticky="w", pady=(0, 3))
        e_sat = ctk.CTkEntry(form, width=165, height=32, placeholder_text=T("aq_ph_sat"))
        e_sat.grid(row=9, column=1, sticky="w", padx=(8, 0), pady=(0, 3))
        ...
        _rxadd(ctk.CTkLabel(form, text=T("aq_sat_rx_hdr"), anchor="w",
               ...
               row=15, column=0, columnspan=2, sticky="w", pady=(6, 2))
        _rxadd(ctk.CTkLabel(form, text=T("aq_sat_rx_banda"), anchor="w",
                     font=ctk.CTkFont(size=11)), row=16, column=0, sticky="w")
        _rxadd(ctk.CTkLabel(form, text=T("aq_sat_rx_freq"), anchor="w",
                     font=ctk.CTkFont(size=11)), row=16, column=1, sticky="w", padx=(8, 0))
        ...
        e_freq_rx = _rxadd(ctk.CTkEntry(form, width=140, placeholder_text="—"),
                     row=17, column=1, sticky="w", padx=(8, 0), pady=(0, 6))
        _rxadd(ctk.CTkLabel(form, text=T("aq_sat_mode"), anchor="w",
                     font=ctk.CTkFont(size=11)), row=18, column=0, sticky="w")
        e_sat_mode = _rxadd(ctk.CTkEntry(form, width=140, placeholder_text="V/U"),
                     row=19, column=0, sticky="w", pady=(0, 3))
        def _mostra_rx(show):
            for _w, _g in _rxw:
                if show:
                    _w.grid(**_g)
        ...
        e_rst_tx = ctk.CTkEntry(form, width=165, height=32)
        e_rst_tx.grid(row=12, column=0, sticky="w", pady=(0, 3))
        e_rst_rx = ctk.CTkEntry(form, width=165, height=32)
        e_rst_rx.grid(row=12, column=1, sticky="w", padx=(8, 0), pady=(0, 3))
        ...
        ctk.CTkLabel(form, text=T("addqso_locator"), anchor="w",
                     font=ctk.CTkFont(size=11)).grid(row=3, column=1, sticky="w", padx=(8, 0))
        e_loc = ctk.CTkEntry(form, width=165, height=32, placeholder_text=T("aq_ph_loc"))
        e_loc.grid(row=4, column=1, sticky="w", padx=(8, 0), pady=(0, 3))

        # Campi extra (compilati anche dal lookup): nome, QTH, zone, stato.
        extra = ctk.CTkFrame(form, fg_color="transparent")
        extra.grid(row=20, column=0, columnspan=2, sticky="w", pady=(6, 6))
        def _ef(rr, cc, lblkey, width=160):
            ctk.CTkLabel(extra, text=T(lblkey), width=64, anchor="e",
                         font=ctk.CTkFont(size=11)).grid(row=rr, column=cc*2, sticky="e",
                                                         padx=(0, 4), pady=2)
            e = ctk.CTkEntry(extra, width=width)
            e.grid(row=rr, column=cc*2+1, sticky="w", padx=(0, 10), pady=2)
            return e
        ...
        # Area centrale: a sinistra i campi (scorrevoli), a destra "QSO precedenti".
