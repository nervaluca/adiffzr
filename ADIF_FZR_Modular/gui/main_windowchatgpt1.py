...
class ADIFtoPDFApp(ctk.CTk):
    ...
    def crea_interfaccia(self):
        ...
        def _navbtn(icon, key, cmd, primary=False):
            b = ctk.CTkButton(
                ...
                fg_color=("#2c7be5" if primary else "transparent"),
                hover_color=(("#1b64c9", "#1b64c9") if primary else _NAV_HOVER),
                text_color=(("#FFFFFF", "#FFFFFF") if primary else ("#1A365D", "#CBD5E1")))
    ...
    def apri_aggiungi_qso(self):
        ...
        qb_side = ctk.CTkFrame(body, fg_color=("#EEF3F8", "#141C25"), corner_radius=10, width=360, border_width=1, border_color=("#D5DEE8", "#26384A"))
        form = ctk.CTkFrame(body, fg_color=("#F8FAFC", "#101820"), corner_radius=10, border_width=1, border_color=("#D9E2EC", "#26384A"))
        ...
        e_data = ctk.CTkEntry(form, width=220, height=32)
        e_data.grid(row=1, column=0, sticky="ew", pady=(0, 2))
        ...
        def _aggiorna_qso_before(call):
            ...
            try:
                ...
                for q in self.qsos_caricati:
                    ...
                    if c and (c == call or _call_base(c) == base):
                        trovati.append(q)
                ...
                if not trovati:
                    ...
                    qb_summary.configure(
                        ...
                        text_color="#5EA0E0")
                    # Slot nuovi rispetto a banda/modo attualmente selezionati
                    try:
                        ...
                        cur_b = cur_m = ""
                    _nuovi = []
                    if cur_b and cur_b not in _bset:
                        _nuovi.append(T("aq_b4_banda_nuova", b=cur_b))
                    if cur_m and cur_m not in _mset:
                        _nuovi.append(T("aq_b4_modo_nuovo", m=cur_m))
                    if _nuovi:
                        qb_slot.configure(text="   ".join(_nuovi), text_color="#48BB78")
                    else:
                        qb_slot.configure(text=T("aq_b4_gia_fatto"), text_color="#8794a3")
                    for q in trovati[:15]:
                        ...
                        conf = ""
                        if self._get_campo_qso(q, "lotw_qsl_rcvd").upper() == "Y":
                            conf = "LoTW"
                        elif self._get_campo_qso(q, "qsl_rcvd").upper() == "Y":
                            conf = "QSL"
                        elif self._get_campo_qso(q, "eqsl_qsl_rcvd").upper() == "Y":
                            conf = "eQSL"
        ...
        # ── CONSOLE RADIO ───────────────────────────────────────────────
        # Non è un semplice riempitivo: mostra in tempo reale i parametri
        # del QSO che verranno registrati e lo stato dell'ultima lettura
        # OmniRig. In questo modo la zona centrale resta utile anche quando
        # si lavora senza CAT.
        radio_card = ctk.CTkFrame(
            form, fg_color=("#EAF4FF", "#142536"), corner_radius=10,
            border_width=1, border_color=("#C7D9EA", "#294A67"))
        radio_card.grid(row=13, column=0, columnspan=4, sticky="nsew", padx=0, pady=(10, 8))
        radio_card.grid_columnconfigure((0, 1, 2, 3), weight=1)

        ctk.CTkLabel(radio_card, text="📻  CONSOLE RADIO", anchor="w",
                     font=ctk.CTkFont(size=13, weight="bold"),
                     text_color=("#1A4F7A", "#7CC7FF")).grid(
                         row=0, column=0, columnspan=4, sticky="ew", padx=14, pady=(9, 5))

        _radio_vars = {
            "band": _tk.StringVar(value=var_banda.get().upper()),
            "freq": _tk.StringVar(value=e_freq.get().strip() or "—"),
            "mode": _tk.StringVar(value=var_modo.get().upper()),
            "rst": _tk.StringVar(value=f"{e_rst_tx.get().strip() or '—'} / {e_rst_rx.get().strip() or '—'}"),
        }
        _radio_cat = _tk.StringVar(value="CAT: standby")

        def _radio_sync(*_):
            _radio_vars["band"].set(var_banda.get().strip().upper() or "—")
            _radio_vars["freq"].set(e_freq.get().strip() or "—")
            _radio_vars["mode"].set(var_modo.get().strip().upper() or "—")
            _radio_vars["rst"].set(f"{e_rst_tx.get().strip() or '—'} / {e_rst_rx.get().strip() or '—'}")

        for _v in (var_banda, var_modo):
            try:
                _v.trace_add("write", _radio_sync)
            except Exception:
                pass
        for _e in (e_freq, e_rst_tx, e_rst_rx):
            _e.bind("<KeyRelease>", _radio_sync, add="+")

        _radio_items = [
            ("BANDA", _radio_vars["band"]),
            ("FREQUENZA", _radio_vars["freq"]),
            ("MODO", _radio_vars["mode"]),
            ("RST TX / RX", _radio_vars["rst"]),
        ]
        for _cc, (_cap, _var) in enumerate(_radio_items):
            _box = ctk.CTkFrame(radio_card, fg_color=("#DCEEFF", "#19334A"), corner_radius=7)
            _box.grid(row=1, column=_cc, sticky="ew", padx=(10 if _cc == 0 else 4, 4), pady=(0, 7))
            ctk.CTkLabel(_box, text=_cap, font=ctk.CTkFont(size=9, weight="bold"),
                         text_color=("#54708A", "#8FB5D3")).pack(anchor="w", padx=9, pady=(5, 0))
            ctk.CTkLabel(_box, textvariable=_var, anchor="w",
                         font=ctk.CTkFont(size=13, weight="bold"),
                         text_color=("#17324D", "#E7F4FF")).pack(fill="x", padx=9, pady=(0, 6))

        _cat_row = ctk.CTkFrame(radio_card, fg_color="transparent")
        _cat_row.grid(row=2, column=0, columnspan=4, sticky="ew", padx=12, pady=(0, 8))
        ctk.CTkLabel(_cat_row, textvariable=_radio_cat, anchor="w",
                     font=ctk.CTkFont(size=10, weight="bold"),
                     text_color=("#5B6B7A", "#AFC7DA")).pack(side="left")
        ctk.CTkLabel(_cat_row, text="  •  I dati vengono aggiornati mentre compili il QSO", anchor="w",
                     font=ctk.CTkFont(size=10), text_color=("#718096", "#829BB0")).pack(side="left")
        ...
        extra.grid(row=7, column=2, columnspan=2, sticky="nw", pady=(2, 6), padx=(14, 0))
        def _ef(rr, cc, lblkey, width=115):
            ctk.CTkLabel(extra, text=T(lblkey), width=55, anchor="e",
                         font=ctk.CTkFont(size=11)).grid(row=rr, column=cc*2, sticky="e",
                                                         padx=(0, 4), pady=2)
            e = ctk.CTkEntry(extra, width=width)
            e.grid(row=rr, column=cc*2+1, sticky="w", padx=(0, 5), pady=2)
            return e
        e_nome  = _ef(0, 0, "f_nome"); e_qth = _ef(0, 1, "f_qth")
        e_cqz   = _ef(1, 0, "f_cqz", 80); e_ituz = _ef(1, 1, "f_ituz", 80)
        e_state = _ef(2, 0, "f_state", 80)
        ...
        qb_side.pack(side="right", fill="y", padx=(10, 0), pady=2)
        qb_side.pack_propagate(False)
