...
class ADIFtoPDFApp(ctk.CTk):
    ...
    def apri_aggiungi_qso(self):
        ...
        try:
            ...
            if str(_pf.get("addqso_modo", "live")).lower().startswith("man"):
        ...
        _modo_seg_var = _tk.StringVar(value=_LBL_LIVE if _modo_iniziale == "live" else _LBL_MAN)

        def _applica_modo(scelta):
            man = (scelta == _LBL_MAN)
            _modo_live[0] = "manuale" if man else "live"
            if man:
                live_var.set(False)
                try:
                    e_data.delete(0, "end")
        ...
        ctk.CTkSegmentedButton(form, values=[_LBL_LIVE, _LBL_MAN],
                               variable=_modo_seg_var, command=_applica_modo,
                               width=280, height=32, dynamic_resizing=False,
                               font=ctk.CTkFont(size=12, weight="bold")).grid(
        ...
        _applica_modo(_modo_seg_var.get())   # applica la modalità iniziale (Live/Manuale)
