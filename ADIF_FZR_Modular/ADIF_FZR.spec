...
# Build ONEFILE di ADIF FZR 2.6 (struttura a moduli)
...
hiddenimports += [
    ...
    "config", "main", "theme",
    "gui", "gui.main_window", "gui.widgets", "gui.was_status", "gui.awards", "gui.award_map", "gui.award_diploma",
    ...
    "net", "net.dxcluster", "net.hamqth", "net.uploaders", "net.wsjtx", "net.lotw_align", "net.qrz", "net.fzr_alert",
    "pdf", "pdf.canvas",
    "radio", "radio.bandplan", "radio.omnirig", "radio.sdrconsole", "radio.satellite", "radio.coastlines", "radio.sat_db",
    "utils", "utils.dxcc", "utils.dxcc_countries", "utils.db", "utils.formatting", "utils.maidenhead", "utils.tooltip", "utils.log_profili", "utils.cty",
]
...
# --- cty.dat + bandierine (flags/) nel pacchetto ---
_cty = trova("cty.dat")
if _cty:
    datas.append((_cty, "."))
for _d in (BASE, PKG):
    _fd = os.path.join(_d, "flags")
    if os.path.isdir(_fd):
        for _f in os.listdir(_fd):          # flags bundle
            if _f.lower().endswith(".png"):
                datas.append((os.path.join(_fd, _f), "flags"))
        break
